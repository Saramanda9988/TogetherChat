package com.luna.webrtcserver.common.domain.vo.response;
...
public class PageBaseResp<T> {
    ...
    public PageBaseResp(List<T> records, long total,  Integer pageSize, Integer pageNo) {
        this.pageNo = pageNo;
        this.pageSize = pageSize;
        this.totalRecords = total;
        this.isLast = isLastPage(total, pageNo, pageSize);
        this.list = records;
    }
    ...
    /**
     * 创建一个PageBaseResp对象（与init方法作用相同，提供便捷别名）
     */
    public static <T> PageBaseResp<T> build(IPage<T> page) {
        return init(page);
    }
     ...
     * 创建一个PageBaseResp对象（与init方法作用相同，提供便捷别名）
     */
    public static <T> PageBaseResp<T> build(IPage page, List<T> list) {
        return init(page, list);
    }
